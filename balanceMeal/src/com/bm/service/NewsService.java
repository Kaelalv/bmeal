...
public class NewsService {
	/* --- 第一步 ：每日一餐的 添加 --- */
	// 2. 验证添加的标题是否存在
	public int checkExit(String newstit){
		String sql = "select count(*) as num from news where newstit = ?";
		//DBUtil.query(sql,mealtit)这里查询出来的结果是map类型，所有通过查询结果来获取“num”的值
		return Integer.parseInt(DBUtil.query(sql,newstit).get("num").toString());
	}
	// 1.写添加每日一餐的方法	
	public void newsAdd(Map<String,Object> map){
		
		// 1.1 写完插入数据后，发现需要检查插入的数据是否已存在，所以要写第2步的内容，
		// 因此下面调用checkExit()这个方法
		int num = checkExit(map.get("newstit").toString());
		if(num==0){// 说明当前没有该账号，可以添加
			String sql = "insert into news (newstit,newsintro,newsdetail,newsimg,newspublish)values(?,?,?,?,?)";
	...
	/*---- 4. 分页查询用户列表 ----------- */
	public PageBean getUserList(News news,PageBean pageBean){
		...
		if(news.getNewstit()!=null && news.getNewstit().trim().length()>0){
			sb.append(" and newstit like '%"+ StringEscapeUtils.escapeSql(news.getNewstit()) +"%'");
		}
		
		if(news.getNewspublish()!=null){
			System.out.println("111111111111");
			sb.append(" and newspublish = '"+ news.getNewspublish() +"'");
		}
	...
	public void updateNewsById(News news){
		String sql = "update news set newstit = ? , newsintro = ? , newsdetail = ? where id = ?";
